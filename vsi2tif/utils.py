...
def _vsi2tif(bleach_correct: bool, split_colors: bool, img_list: []):
    ...
    for img in img_list:
        ...
        if bleach_correct:
            image_temp = AICSImage(
                histogram_bleach_correct(image.data),
        ...
        if split_colors:
            for i in range(len(image.channel_names)):
                ...
                image_temp = AICSImage(
                    image.get_image_data("TZCYX", C=i),
                    physical_pixel_sizes=image.physical_pixel_sizes,
                )

                imwrite(
                    img.with_name(f"{img.stem}_{ch!s}.tif"),
                    image_temp.data,
                    imagej=True,
                    resolution=(
                        1.0 / image_temp.physical_pixel_sizes.X,
                        1.0 / image_temp.physical_pixel_sizes.Y,
                    ),
                    resolutionunit="MICROMETER",
                    metadata={
                        "spacing": image_temp.physical_pixel_sizes.Z,
                        "unit": "um",
                        "axes": "TZCYX",
                    },
                )
        ...
        for i in range(z):
            ...
            if i > 0:
                ...
                val, ix, cnt = np.unique(
                    stack[i].flatten(), return_inverse=True, return_counts=True
                )
            ...
            val, cnt = np.unique(stack[i].flatten(), return_counts=True)
            cdf = np.cumsum(cnt) / pixel_number
            values.append(val)
            cdfs.append(cdf)
